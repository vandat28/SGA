...
export const LessonPlanDisplay: React.FC<LessonPlanDisplayProps> = ({
  ...
  const handlePrint = () => {
    ...
    if (printWindow && contentRef.current) {
      ...
      printWindow.document.write(
        "<style> @page { size: A4; margin: 1in; } body { font-family: sans-serif; } .prose { max-width: 100% !important; } .prose table { border-collapse: collapse; width: 100%; } .prose th, .prose td { border: 1px solid #ccc; padding: 8px; } </style>"
      );
      printWindow.document.write('</head><body class="p-4">');
      printWindow.document.write(
        `<div class="prose prose-sm max-w-none">${contentRef.current.innerHTML}</div>`
      );
  ...
  const handleDownloadWord = () => {
    if (!contentRef.current) {
      console.error("Content ref is not available for download.");
      return;
    }

    const contentHTML = contentRef.current.innerHTML;

    // Lấy các style từ component của bạn và từ hàm print
    // Điều này rất quan trọng để Word có thể hiển thị đúng
    const styles = `
      <style>
        /* CSS từ hàm print */
        @page { size: A4; margin: 1in; }
        body { font-family: sans-serif; padding: 1in; } /* Thêm padding cho body */
        .prose { max-width: 100% !important; }
        
        /* CSS từ thẻ <style> của component */
        .prose table {
          width: 100%;
        ...
        .prose table p:first-child { margin-top: 0; }
        .prose table p:last-child { margin-bottom: 0; }
      </style>
    `;

    // Tạo một tài liệu HTML đầy đủ
    // Chúng ta thêm link CDN của Tailwind để Word có thể tải và áp dụng các class 'prose'
    const fullHtml = `
      <!DOCTYPE html>
      <html lang="vi">
        <head>
    ...
    // Tạo một Blob với kiểu MIME của Word
    const blob = new Blob([fullHtml], {
      type: "application/msword;charset=utf-8",
    });

    // Sử dụng saveAs để kích hoạt tải xuống
    saveAs(blob, "GiaoAn.doc");
  };
      ...
      return (
        <div className="flex flex-col items-center justify-center h-full text-center">
          ...
          <p className="mt-2 text-sm text-slate-500">
            Nội dung sẽ xuất hiện ngay khi có. Vui lòng chờ.
          </p>
        </div>
      );
    }
    ...
    return (
      <div className="flex flex-col items-center justify-center h-full text-center text-slate-500">
        ...
        <h3 className="text-lg font-semibold text-slate-700">
          Khu vực hiển thị giáo án
        </h3>
        <p className="max-w-md mt-2">
          Giáo án sẽ được hiển thị ở đây sau khi bạn nhập thông tin, tải ảnh lên
          và nhấn "Tạo Giáo Án".
        </p>
      </div>
    );
          ...
          <div className="flex items-center space-x-1">
            {isEditing ? (
              <>
                <button
                  onClick={handleSave}
                  className="p-2 rounded-md text-slate-500 hover:bg-slate-100 hover:text-green-600 transition-colors"
                  title="Lưu thay đổi"
                >
                  <SaveIcon />
                </button>
              ...
              <>
                <button
                  onClick={handleEdit}
                  className="p-2 rounded-md text-slate-500 hover:bg-slate-100 hover:text-indigo-600 transition-colors"
                  title="Chỉnh sửa"
                >
                  <EditIcon />
                </button>
